...
async def generate_from_base64(request: GenerateRequest) -> GenerateResponse:
    ...
    try:
        ...
        result.ply_file_base64 = base64.b64encode(ply_bytes).decode("utf-8") if ply_bytes else None
...
async def generate_spz(prompt_image_file: UploadFile = File(...), seed: int = Form(-1)) -> StreamingResponse:
    ...
    try:
        ...
        if not ply_bytes:
            raise HTTPException(status_code=500, detail="Failed to generate PLY file")
